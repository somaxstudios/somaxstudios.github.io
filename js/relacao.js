...
// DOM elements (serão injetados na função init)
let elements = {};
...
// ---------- FILTROS (NOVAS OPÇÕES) ----------
function aplicarFiltroStatus(produtos) {
  const opcao = elements.statusFilter.value;
  // Opções:
  // 'todos-com-prateleira'  -> todos os produtos que têm prateleira (independente do status)
  // 'off-com-prateleira'    -> OFF e prateleira preenchida
  // 'off-sem-prateleira'    -> OFF e prateleira vazia
  // 'on-com-prateleira'     -> ON e prateleira preenchida
  // 'on-sem-prateleira'     -> ON e prateleira vazia
  // 'com-identificacao'     -> todos com título, artista e gravadora preenchidos
  // 'sem-identificacao'     -> todos com pelo menos um campo vazio (título, artista ou gravadora)
...
function aplicarFiltrosCompletos() {
  ...
  // 6. Identificação (este filtro é redundante com as novas opções, mas mantido para compatibilidade)
  // Podemos deixar ou remover. Vou manter para não quebrar, mas ele pode ser removido futuramente.
  // O filtro 'identificacaoFilter' será aplicado adicionalmente.
...
export function init() {
  // Captura todos os elementos DOM
  elements = {
